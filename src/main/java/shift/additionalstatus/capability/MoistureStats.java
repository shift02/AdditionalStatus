...
public class MoistureStats {
	...
	public void addStats(EntityPlayer entityPlayer, int par1, float par2) {
		...
		if (!ASConfig.statusMoisture) return;
		...
		if (MinecraftForge.EVENT_BUS.post(event)) {
			...
			this.moistureLevel = MathHelper.clamp_int(event.newAddMoistureLevel + this.moistureLevel, 0,
					MAX_MOISTURE_LEVEL);
			this.moistureSaturationLevel = MathHelper.clamp_float(
	...
	public void onUpdate(EntityPlayer par1EntityPlayer) {
		...
		if (this.moistureExhaustionLevel > 4.0F) {
			...
			} else if (i.getDifficultyId() > 0 || ASConfig.peacefulMoisture) {
		...
		if (par1EntityPlayer.worldObj.getGameRules().getBoolean("naturalRegeneration")
			...
			if (this.moistureTimer >= 80) {
				if (par1EntityPlayer.getHealth() > 10.0F || i.getDifficultyId() >= 3
					...
					// 1.0F);
					AdditionalStatusAPI.addStaminaExhaustion(par1EntityPlayer, 0.5f);
				}
	...
	public boolean isPacket() {
		...
		if (!ASConfig.statusMoisture) return false;
	...
	public void addExhaustion(EntityPlayer entityPlayer, float par1) {
		...
		if (MinecraftForge.EVENT_BUS.post(event)) {
			...
			this.moistureExhaustionLevel = MathHelper
					.clamp_float(event.newExhaustionLevel + this.moistureExhaustionLevel, 0, 40.0F);
		}
