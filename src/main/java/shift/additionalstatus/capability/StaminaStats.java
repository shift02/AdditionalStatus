...
import shift.additionalstatus.api.event.player.PlayerMoistureEvent;
import shift.additionalstatus.api.event.player.PlayerStaminaEvent;
...
public class StaminaStats {
	...
	public void addStats(EntityPlayer entityPlayer, int par1, float par2) {
		...
		if (!ASConfig.statusStamina) return;
		...
		if (MinecraftForge.EVENT_BUS.post(event)) {
			...
			this.staminaLevel = MathHelper.clamp_int(event.newAddMoistureLevel + this.staminaLevel, 0,
					MAX_STAMINA_LEVEL);
			this.staminaSaturationLevel = MathHelper.clamp_float(
	...
	public void onUpdate(EntityPlayer par1EntityPlayer) {
		...
		EnumDifficulty i = par1EntityPlayer.worldObj.getDifficulty();
		...
		if (this.staminaExhaustionLevel > 4.0F) {
			...
			} else if (i.getDifficultyId() > 0 || ASConfig.peacefulStamina) {
				this.staminaLevel = Math.max(this.staminaLevel - 1, 0);
			}
		}

		if (par1EntityPlayer.worldObj.getGameRules().getBoolean("naturalRegeneration")
	...
	public boolean isPacket() {
		...
		if (!ASConfig.statusStamina) return false;
	...
	public int getStaminaLevel() {
		...
		if (!ASConfig.statusStamina) return MAX_STAMINA_LEVEL;
	...
	public void addExhaustion(EntityPlayer entityPlayer, float par1) {
		...
		if (MinecraftForge.EVENT_BUS.post(event)) {
			...
			this.staminaExhaustionLevel = MathHelper.clamp_float(event.newExhaustionLevel + this.staminaExhaustionLevel,
					0, 400.0F);
		}
	...
	public float getSaturationLevel() {
		...
		if (!ASConfig.statusStamina) return MAX_STAMINA_LEVEL / 3.0F;
